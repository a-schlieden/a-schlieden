...
let tags = document.querySelectorAll('.grid_wrap button');
...
for (item of tags) {
    this.item.onclick = function () {
        ...
        if (this.id = elemId) {
            ...
            document.querySelector('.grid_wrap').style.display = "none";
        }
    }
}
...
for (item of closeBtn) {
    this.item.onclick = function () {
        ...
        document.querySelector('.grid_wrap').style.display = "grid";
    }
}
...
let hoverImages = document.querySelectorAll('.works_img');

for (let i = 0; i < hoverImages.length; i++) {
    hoverImages[i].onmouseenter = () => {
        console.log(hoverImages[i].nextElementSibling);
        hoverImages[i].classList.add("works_img_aktive");
        hoverImages[i].nextElementSibling.classList.add("works_skill_aktive");
    }
    hoverImages[i].onmouseleave = () => {
        hoverImages[i].classList.remove("works_img_aktive");
        hoverImages[i].nextElementSibling.classList.remove("works_skill_aktive");
    }
}
